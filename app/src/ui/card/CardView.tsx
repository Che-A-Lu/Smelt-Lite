...
export function CardView({
  ...
  const finishDrag = useCallback(() => {
    const d = ds.current;
    d.active = false; cancelAnimationFrame(raf.current); isDragging.current = false;
    ...
    el.style.zIndex = "";
    setTimeout(() => { if (el) { el.style.boxShadow = "none"; el.style.transition = ""; } }, 150);
  ...
  }, [editing, deleting, interaction, card, onClick]);
    ...
    <>
      <div
        ...
        onMouseEnter={() => {
          ...
          const el = elRef.current; if (!el) return;
          el.style.transform = `translate(${card.position.x}px, ${card.position.y - 4}px)`;
          el.style.boxShadow = "0 4px 12px rgba(0,0,0,0.1)";
        }}
        onMouseLeave={() => {
          if (isDragging.current) return;
          const el = elRef.current; if (!el) return;
          el.style.transform = `translate(${card.position.x}px, ${card.position.y}px)`;
      ...
      >
        <div style={{ position: "absolute", left: 0, top: `${4 * cardRem}px`, bottom: `${4 * cardRem}px`, width: `${3 * cardRem}px`, borderRadius: `${2 * cardRem}px 0 0 ${2 * cardRem}px`, background: lineColor, transition: "background 150ms ease-out" }} />
        <div style={{ position: "absolute", left: `${8 * cardRem}px`, top: `${8 * cardRem}px`, width: `${6 * cardRem}px`, height: `${12 * cardRem}px`, borderRadius: `${1 * cardRem}px`, background: card.isWorkbench ? "#1a1a2e" : category.color }} />

        {editing ? (
          <input autoFocus value={editValue} onChange={(e) => setEditValue(e.target.value)} onBlur={commitEdit} onKeyDown={onEditKeyDown}
            style={{ position: "absolute", left: `${20 * cardRem}px`, top: `${6 * cardRem}px`, width: `${220 * cardRem}px`, border: "1px solid #3b82f6", borderRadius: `${2 * cardRem}px`, fontSize: `${10 * cardRem}px`, padding: "1px 4px", outline: "none", fontFamily: "inherit" }} />
        ) : (
          <div style={{ position: "absolute", left: `${20 * cardRem}px`, top: `${8 * cardRem}px`, right: `${8 * cardRem}px`, fontSize: `${10 * cardRem}px`, color: "#1a1a2e", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {card.label || t("card.defaultLabel")}
          </div>
        )}

        {/* 预览区 */}
        <div style={{ position: "absolute", left: `${8 * cardRem}px`, top: `${previewTop * cardRem}px`, right: `${8 * cardRem}px`, bottom: `${previewBottom * cardRem}px` }}>
          {snapshotUrl ? (
            <img src={snapshotUrl} style={{ width: "100%", height: "100%", objectFit: "contain", borderRadius: `${2 * cardRem}px` }} alt="" />
          ) : snapLoading ? (
            <div style={{ fontSize: `${9 * cardRem}px`, color: "#d1d5db", textAlign: "center", paddingTop: `${30 * cardRem}px` }}>...</div>
          ) : card.sphereId ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%" }}>
              <span style={{ fontSize: `${20 * cardRem}px`, opacity: 0.3 }}>{category.label}</span>
            </div>
          ) : null}
        </div>
        ...
        {/* 标签区（底部） */}
        {card.tags.length > 0 && (
          <div style={{ position: "absolute", left: `${8 * cardRem}px`, right: `${8 * cardRem}px`, bottom: `${6 * cardRem}px`, fontSize: `${8 * cardRem}px`, color: "#9ca3af", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", lineHeight: 1.2 }}>
            {card.tags.join(", ")}
          </div>
        )}

        {card.isWorkbench && (
          <div style={{ position: "absolute", right: `${4 * cardRem}px`, bottom: `${4 * cardRem}px`, width: `${8 * cardRem}px`, height: `${8 * cardRem}px`, borderRadius: "50%", background: "#1a1a2e" }} />
        )}
      </div>
      ...
      {/* 右键菜单（Portal 到 body，避开 Canvas transform） */}
      {ctxMenu && card.sphereId && createPortal(
